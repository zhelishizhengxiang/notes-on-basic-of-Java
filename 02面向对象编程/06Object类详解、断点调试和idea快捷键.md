...
##### (2)equals方法
对于equals方法：
1. 他是**Object类的普通方法，只能判断引用类型，默认判断的是对象地址是否相等，即判断两个对象是否是同一个对象**
...
2. **子类往往回重写该方法，用于判断内容是否相等。比如Integer、String**
...
3. 如果一个类没有重写equals方法，那么他是继承的Object类的equals方法，也就是判断两个对象是否是同一个对象
...
3. 两个引用，如果指向的是不同的对象，则哈希值是不一样
4. 哈希值主要是根据对象的内部地址转换成一个整数得来，不能完全将哈希值等价于地址
5. 后面在集合中，如果需要的话，此方法也会进行重写
...
```
1. 该方法返回该对象的字符串表示，默认返回全类名（包名+类名）+@+对象的hashcode值的16进制表示
...
2. 子类往往重写toString方法用于返回对象的属性信息，idea中默认的重写后的代码如下所示
```java
@Override  
public String toString() {  
	return "B{" +  
...
1. 当对象被回收时，系统自动调用该对象的finalize方法。**子类可以重写该方法，做一些释放资源的操作**（比如释放资源：数据库连接、或者打开文件）  
2. 什么时候回收：当某个对象没有任何引用时，则jvm就认为这个对象是一个垃圾对象，就会使用垃圾回收机制来销毁该对象，在销毁该对象前，就会调用finalize（垃圾回收器并不是在对象一变成垃圾就立马回收，而是由自己的一套算法，可能会程序结束后销毁对象）
...
## 3）快捷键汇总：
1. `fori`可以快速写出for循环
2. `new 类名().var`可以快速创建对象
3. ctrl+alt+F可以快速格式化代码
4. 光标放在对应的类，按ctrl+H可以查看类关系，也可以使用图表来进行查看
5. 删除当前行 ctrl+d
6. 数字.for就会直接出现到数字的for循环：比如10000.for