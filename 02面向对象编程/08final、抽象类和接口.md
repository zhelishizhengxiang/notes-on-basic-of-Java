...
```java
访问修饰符 安abstract 返回类型 方法名(参数列表);//没有方法体，没有大括号
```
3. 抽象类的价值更多作用在于设计，是设计者设计好后，让子类继承并实现抽象类()

使用细节：
1. **抽象类不能被实例化**，抽象类只能用来被继承
2. 抽象类不一定要包含abstract方法，也就是说，抽象类可有没有abstract方法
...
* JDK8后接口可以有静态方法，默认方法、也就是说接口中可又方法的具体实现。具体的例子如下图所示：
